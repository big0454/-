...
def extract_angpao_codes(text):
    ...
    matches = re.findall(pattern, text)  # จับลิงก์ปกติ

    # 📌 ตรวจสอบคำที่อาจมีลิงก์ติดกับตัวอักษรอื่น
    words = text.split()
    for word in words:
...
# 📌 ฟังก์ชันส่ง API รับเงิน (ลด timeout ให้ไวสุด ๆ)
...
# 📌 ดักจับข้อความทุกประเภท (ข้อความธรรมดา + ลิงก์สีฟ้า + ลิงก์ที่ติดข้อความ)
